    ...
    public void convert(final ExchangeConversionInputData inputData) {
        try {
            ...
            final Map<String, Double> rates =
                    exchangeDataAccess.getRates(inputData.getFrom());
            final Double rate = rates.get(inputData.getTo());
            if (rate == null) {
                exchangePresenter.presentConversionFailure("Invalid target currency.");
                return;
            ...
            ConvertTransaction tx = new ConvertTransaction(
            ...
            transactionDAO.save(inputData.getUsername(), tx);

            final List<SubAccount> updatedSubAccounts =
                    exchangeDataAccess.getSubAccountsOf(inputData.getUsername());
